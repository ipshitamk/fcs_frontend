...
#       filter(SSP == ssp)
#     
#     plot_ly(temp_subset, x = ~date, y = ~Temperature, color = ~Variable, linetype = ~Variable) %>%
#       add_lines(showlegend = TRUE) %>%
#       layout(title = paste("Temperature in", city, "for", ssp),
...
#   multiplot <- subplot(plots_temp, nrows = 2)
#   
#   # Show the plot
#   multiplot
# }
