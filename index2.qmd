...
ahe_maps <-plot_basemap("vector")
uhi_maps <-plot_basemap("vector")
ul_maps <-plot_basemap("vector")
...
# Define options
options <- list(
  list(year = '2050', ssp = 'SSP1'),
  list(year = '2050', ssp = 'SSP2'),
  list(year = '2050', ssp = 'SSP3'),
  list(year = '2050', ssp = 'SSP4'),
  list(year = '2050', ssp = 'SSP5'),
  list(year = '2100', ssp = 'SSP1'),
  list(year = '2100', ssp = 'SSP2'),
  list(year = '2100', ssp = 'SSP3'),
  list(year = '2100', ssp = 'SSP4'),
...
for (option in options) {
  dropdownItem <- tags$a(
    class = "dropdown-item",
    href = "#",
    onclick = paste0("selectYearAndSSP('", option$year, "', '", option$ssp, "')"),
    paste0(option$year, ", ", option$ssp)
  )
  dropdownMenu <- tagAppendChild(dropdownMenu, dropdownItem)
}
...
```{r}
# batch pop
# add_pop_layer <- create_layer_function(data = pop, yaml_key = "pop",group_id="population-density-all",title="Population Density")
# add_pop1_layer <- create_layer_function(data = pop1, yaml_key = "pop",group_id="population-density-all",title="Population Density")
# add_pop2_layer <- create_layer_function(data = pop2, yaml_key = "pop",group_id="population-density-all",title="Population Density")
# add_pop3_layer <- create_layer_function(data = pop3, yaml_key = "pop",group_id="population-density-all",title="Population Density")
pop_all_maps <- add_pop_layer(pop_maps)
pop_all_maps <- add_pop1_layer(pop_maps)
pop_all_maps <- add_pop2_layer(pop_maps)
pop_all_maps <- add_pop3_layer(pop_maps)
pop_layers <- c(pop_layers,"population-density-all")
pop_maps <- pop_all_maps %>%
  add_aoi() %>%
  addLayersControl(
    overlayGroups = pop_layers,
...
```{r}
#batch GDP
gdp_all_maps <- add_gdp_layer(gdp_maps)
gdp_all_maps <- add_gdp1_layer(gdp_maps)
gdp_all_maps <- add_gdp2_layer(gdp_maps)
gdp_all_maps <- add_gdp3_layer(gdp_maps)
gdp_layers <- c(gdp_layers,"gdp-all")
gdp_maps <- gdp_all_maps %>%
  add_aoi() %>%
  addLayersControl(
    overlayGroups = gdp_layers,
...
all_maps_list <- list(pop_maps,gdp_maps)
```
### Feedback
```{ojs}
//| output: true
