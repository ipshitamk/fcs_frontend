...
## Learn more about Housing and Urban Development
Do we need to add this for FCS?
:::

::: {.callout-caution collapse="true"}
## Learn more about Water Supply and Hygiene
Do we need to add this for FCS?
:::
...
::: {.callout-caution collapse="true"}
## Learn more about Disaster Management and Recovery
Do we need to add this for FCS?
:::
...
plot_ly(z = t(m), colorscale = "YlOrRd") %>%
...
plot_ly(z = t(g), colorscale = "YlOrRd") %>%
...
plot_ahe <- layout(plot_ahe,
               title = "Anthropogenic heatflux by SSP Scenario",
               xaxis = list(title = "SSP Scenario"),
...
plot_ly(z = t(u), colorscale = "YlOrRd") %>%
  add_heatmap(x = colnames(u), y = rownames(u)) %>%
  layout(
    xaxis = list(title = "Year"),
...
plot_uhi <- plot_ly(x = colnames(heat)) 
...
### Pluvial Flooding 2020

```{r}
#| include: false
health_points <- fuzzy_read(spatial_dir, "health$", FUN = vect) 
pluvial_low_2020 <- fuzzy_read(spatial_dir, "pluvial_2020_lt1", rast)
pluvial_med_2020 <- fuzzy_read(spatial_dir, "pluvial_2020_1-10", rast) * 10
pluvial_high_2020 <- fuzzy_read(spatial_dir, "pluvial_2020_gt10", rast) * 11

# Interactive Plot (Leaflet)
#pluvial <- max(pluvial_low_2020, pluvial_med_2020, na.rm = T)
add_pluvial_layer <- create_layer_function(data = pluvial_low_2020, yaml_key = "pluvial",group_id="pluvial-flooding-2020")
add_health_points_layer <- create_layer_function(data = health_points, yaml_key = "infrastructure-health",group_id="infrastructure")
all_maps <-  add_health_points_layer(all_maps)
all_maps <-add_pluvial_layer(all_maps)
pu_layers <- c(pu_layers, "pluvial-flooding-2020")
```
...
### Pluvial Flooding SSP2 2050

```{r}
#| include: false
pluvial_low_2050 <- fuzzy_read(spatial_dir, "pluvial_2050_ssp2_lt1", rast)
pluvial_med_2050 <- fuzzy_read(spatial_dir, "pluvial_2050_ssp2_1-10", rast) * 10
pluvial_high_2050 <- fuzzy_read(spatial_dir, "pluvial_2050_ssp2_gt10", rast) * 11

# Interactive Plot (Leaflet)
#pluvial <- max(pluvial_low_2050, pluvial_med_2050, na.rm = T)
add_pluvial1_layer <- create_layer_function(data = pluvial_low_2050, yaml_key = "pluvial",group_id="pluvial-flooding-ssp2-2050")
all_maps <- add_pluvial1_layer(all_maps)
pu_layers <- c(pu_layers, "pluvial-flooding-ssp2-2050")
```
...
### Pluvial Flooding SSP2 2080

```{r}
#| include: false
pluvial_low_2080 <- fuzzy_read(spatial_dir, "pluvial_2080_ssp2_lt1", rast)
pluvial_med_2080 <- fuzzy_read(spatial_dir, "pluvial_2080_ssp2_1-10", rast) * 10
pluvial_high_2080 <- fuzzy_read(spatial_dir, "pluvial_2080_ssp2_gt10", rast) * 11

# Interactive Plot (Leaflet)
#pluvial <- max(pluvial_low_2080, pluvial_med_2080, na.rm = T)
add_pluvial2_layer <- create_layer_function(data = pluvial_low_2080, yaml_key = "pluvial",group_id="pluvial-flooding-ssp2-2080")
all_maps <- add_pluvial2_layer(all_maps) 
pu_layers <- c(pu_layers, "pluvial-flooding-ssp2-2080")
```
...
### Pluvial Flooding SSP5 2050

```{r}
#| include: false
pluvial_med_2050 <- fuzzy_read(spatial_dir, "pluvial_2050_ssp5_1-10", rast) * 10
# Interactive Plot (Leaflet)
#pluvial <- max(pluvial_low_2050, pluvial_med_2050, na.rm = T)
add_pluvial4_layer <- create_layer_function(data = pluvial_med_2050, yaml_key = "pluvial",group_id="pluvial-flooding-ssp5-2050")
all_maps <- add_pluvial4_layer(all_maps)
pu_layers <- c(pu_layers, "pluvial-flooding-ssp5-2050")
```
...
### Pluvial Flooding SSP5 2080

```{r}
#| include: false
pluvial_med_2080 <- fuzzy_read(spatial_dir, "pluvial_2080_ssp5_1-10", rast) * 10

# Interactive Plot (Leaflet)
#pluvial <- max(pluvial_low_2080, pluvial_med_2080, na.rm = T)
add_pluvial5_layer <- create_layer_function(data = pluvial_med_2080, yaml_key = "pluvial",group_id="pluvial-flooding-ssp5-2080")
all_maps <- add_pluvial5_layer(all_maps) 
pu_layers <- c(pu_layers, "pluvial-flooding-ssp5-2080")
```
...
### Fluvial Flooding SSP5 2080

```{r}
#| include: false
fluvial_med_2080 <- fuzzy_read(spatial_dir, "fluvial_2080_ssp5_1-10", rast) * 10

# Interactive Plot (Leaflet)
#pluvial <- max(pluvial_low_2080, pluvial_med_2080, na.rm = T)
add_fluvial_layer <- create_layer_function(data = fluvial_med_2080, yaml_key = "fluvial",group_id="fluvial-flooding-ssp5-2080")
all_maps <- add_fluvial_layer(all_maps) 
fu_layers <- c(fu_layers, "fluvial-flooding-ssp5-2080")
```
...
```{r}
#| include: false
coastal_med_2020 <- fuzzy_read(spatial_dir, "coastal_2020_1-10", rast) * 10

# Interactive Plot (Leaflet)
#pluvial <- max(pluvial_low_2080, pluvial_med_2080, na.rm = T)
add_coastal_layer <- create_layer_function(data = coastal_med_2020, yaml_key = "coastal",group_id="coastal-flooding")
all_maps <- add_coastal_layer(all_maps) 
pu_layers <- c(pu_layers, "coastal-flooding")
```
...
```{r}
added_layers <-c(pop_layers, gdp_layers,ul_layers,ahe_layers,uhi_layers,pu_layers,fu_layers)
```
